...
async function fixRPCFunctions() {
  try {
    console.log('Fixing get_admin_user_list function...');
    
    // First, drop the existing function
    const dropQuery = `DROP FUNCTION IF EXISTS get_admin_user_list;`;
    
    // Create the new function with correct column references
    const createQuery = `
...
$$;`;
    ...
    // Execute using direct SQL query through Supabase client
    console.log('Dropping existing function...');
    const { error: dropError } = await supabase.from('_dummy').select('1').limit(0);
    
    // Since we can't execute raw SQL directly, let's test the RPC function first
    ...
    if (testError) {
      ...
      console.log('The function needs to be fixed in the Supabase dashboard SQL editor.');
